...
import { ItemDoc } from "@/utils/types";
...
export default async function MenuPage({ searchParams }: PageProps) {
  ...
  const filteredItems = selectedCategoryId
    ? items.filter(item => {
      if (typeof item.category === 'object' && item.category && '_id' in item.category) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 justify-center md:justify-start">
        <Link
          href="/admin/menu"
          className={`inline-block py-3 px-8 rounded-xl font-bold uppercase tracking-wider transition ${!selectedCategoryId
        ...
        {categories.map((cat) => (
          <Link
            key={cat._id}
            href={`/admin/menu?category=${cat._id}`}
            className={`inline-block py-3 px-8 rounded-xl font-bold uppercase tracking-wider transition ${selectedCategoryId === cat._id
              ? "bg-blue-500 text-white shadow-lg shadow-blue-500/30"
              : "bg-white/5 text-gray-300 hover:bg-white/10 border border-white/10"
              }`}
          >
